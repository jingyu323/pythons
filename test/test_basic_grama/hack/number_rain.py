...
pygame.init()
winSur = pygame.display.set_mode((init_width, init_height), pygame.FULLSCREEN | pygame.HWSURFACE)
...
while True:
    ...
    for i in range(len(drops)):
        ...
        drops[i] += 1
        if drops[i] * 10 > init_height or random.random() > 0.95:
            drops[i] = 0
    pygame.display.flip()