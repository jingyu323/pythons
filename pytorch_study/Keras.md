...
LOSS（损失函数）是用于衡量[模型预测](https://so.csdn.net/so/search?q=模型预测&spm=1001.2101.3001.7020)结果与实际结果之间差异的函数。通常情况下，LOSS的值应该是一个非负数，因为它是用来量化模型的表现，而非负的LOSS值才能保证模型有改进的空间
